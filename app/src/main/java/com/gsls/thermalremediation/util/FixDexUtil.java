...
public class FixDexUtil {
    ...
    //这下面两个属性可自己修改
    private final static String REPAIR_FILE_NAME = "lingzhiyidexhook";  //修复文件名
//    private final static String REPAIR_FILE_PATH = "1079";      //修复文件路径(默认初始路径为根目录)
     ...
     * @param context
     */
    public static void startRepair(final Context context, String curPkgName) {
        File externalStorageDirectory = Environment.getExternalStorageDirectory();
        //  /sdcard/包名/
        File curPkgNameFile = new File(externalStorageDirectory, curPkgName);
        // 遍历所有的修复dex , 因为可能是多个dex修复包
        File fileDir = externalStorageDirectory != null ?
                new File(externalStorageDirectory, curPkgName) :
                new File(context.getFilesDir(), FixDexUtil.DEX_DIR);// data/user/0/包名/files/odex（这个可以任意位置）
        if (!curPkgNameFile.exists()) {
            curPkgNameFile.mkdirs();
        }
        if (!fileDir.exists()) {//如果目录不存在就创建所有目录，这里需要添加权限
            fileDir.mkdirs();
        }
        if (FixDexUtil.isGoingToFix(context, curPkgName)) {
     ...
     * @desc 验证是否需要热修复
     */
    public static boolean isGoingToFix(@NonNull Context context, String curPkgName) {
